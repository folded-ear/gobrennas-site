...
**npm:**
```bash
npx skills add https://github.com/gohypergiant/agent-skills --skill english-manager
```

**pnpm:**
```bash
pnpm dlx skills add https://github.com/gohypergiant/agent-skills --skill english-manager
```

Select `english-manager` when prompted. This skill is not published as a standalone npm package.
...
```text
/english-manager audit+rewrite in strict mode the following:
...
## What is english-manager?

`english-manager` rewrites English to make it clearer without drifting meaning, tone, audience, or required constraints. Use it for docs, prompts, support replies, release notes, UI copy, policy text, and other writing where clarity matters more than flourish.
...
Source: `skills/english-manager/evals/evals.json`
